...
import { GetLanguageDefinitions } from './languagedefinitions';
    ...
    set language(language: string) {
        ...
        if (this._componentInitialized) {
            if (this._webview) {
                ...
                // Check to see that the language being specified
                // is in the list of known languages
                let languageDefinitions: any = GetLanguageDefinitions();
                let languageIds: any = Object.keys(languageDefinitions);
                for (let i: number = 0; i < languageIds.length; i++) {
                    // Language found. Set up the static stuff to support
                    // syntax highlighting within the editor.
                    if (languageIds[i] === language) {
                        let key: string = languageIds[i];
                        let id: string = languageDefinitions[key].def.id;
                        let def: any = languageDefinitions[key].def;
                        let tokensProvider: any = languageDefinitions[key].tokensProvider;
                        let configuration: any = languageDefinitions[key].configuration;

                        // Register the language
                        monaco.languages.register(def);

                        // Callback to set the tokens provider and config
                        // when the editor language changes.
                        monaco.languages.onLanguage(id, () => {
                            console.log('In onLanguage callback');
                            monaco.languages.setMonarchTokensProvider(id, tokensProvider);
                            monaco.languages.setLanguageConfiguration(id, configuration);
                        });

                        // Have to recreate the editor on language change...
                        ...
                        // Now bootstrap the language server client into the editor.
                        let languageServerClient: any = require('./languageserverclient');
                        languageServerClient.Initialize(this._editor, id);
                    }
                }
            }
        }
